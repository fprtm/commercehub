...
2. **`WHATSAPP_APP_SECRET` unset no longer crashes boot.** It previously did (a deliberate hard requirement, so a deployed build could never silently skip webhook signature verification). Moving it to the DB means it can now legitimately be unset until the owner configures it post-deploy. The accepted risk: `POST /webhook`'s existing `if (appSecret)` skip-verification behavior now has a longer possible unset window — judged acceptable because nobody can have pointed a real Meta webhook at this server yet either (that requires the verify token, entered the same way), so the exposure is "not yet integrated with Meta at all," the same shape of risk `WHATSAPP_MODE=baileys` already accepts today (zero Meta credentials, signature check already skipped in that mode). Flagged here rather than silently decided, per this project's Security Prior Escalation practice — worth a second look if this app ever goes multi-tenant or public-facing before Meta integration is confirmed.
...
## Definition of Done
- [x] FR-1401–FR-1405 implemented and verified
- [x] Full suite: `npx turbo run test --force` → 222/222 passing (was 213; +9 new: 4 `settingsRepo` credential tests + 5 `/settings/credentials` route tests), 5/5 turbo tasks
