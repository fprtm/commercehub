...
// The app's apps/<app-name>/ folder is a flat directory that holds BOTH the
// runnable npm package (package.json, src/, tests/, config/, scripts/, ...)
// AND sibling portfolio/internal material (docs/, business-simulation.md,
// sales-assets.md, portfolio-case-study.md, etc. -- see TICKET-1102/the
// app/-flattening cleanup). Only the former belongs in a client-delivery
// extraction, so the app copy uses this allowlist instead of copying
// everything in the directory: anything not named here (docs/, *.md sales
// collateral, or any other internal file dropped alongside the app in the
// future) is excluded by default rather than needing to be added to a
// blocklist after the fact.
const APP_COPY_TOP_LEVEL_ALLOWLIST = new Set([
  'package.json',
  'package-lock.json',
  'README.md',
  '.env.example',
 ...
 * @param {Set<string>} [opts.topLevelAllowlist] - if set, only entries at the
 *   top level of srcDir (relPath === '') whose name is in this set are
 *   copied; everything else at the top level is skipped. Does not affect
 *   nested entries.
 */
function copyTreeExcluding(srcDir, destDir, opts = {}) {
  ...
  const topLevelAllowlist = opts.topLevelAllowlist || null;
  const skipped = [];
  let copiedCount = 0;
  ...
  function walk(currentSrc, currentDest, relPath) {
    ...
    for (const entry of entries) {
      ...
      if (isTopLevel && topLevelAllowlist && !topLevelAllowlist.has(entry.name)) {
        skipped.push(entry.isDirectory() ? `${entryRel}/ (directory, not part of the app package)` : `${entryRel} (not part of the app package)`);
...
function extractApp(appName, outputDir) {
  ...
  // 1. Copy the app itself (excluding secrets/data, package-lock.json -- a
  // fresh lockfile is generated by `npm install` in the output, since the
  // dependency graph is about to change from workspace-resolved to file:-
  // resolved packages -- and anything at the top level that isn't part of
  // the runnable app package, e.g. docs/ and portfolio/sales markdown that
  // lives alongside the app in its apps/<app-name>/ folder).
  const appCopyResult = copyTreeExcluding(appDir, outputDir, {
    extraSkipExactFilenames: SKIP_FROM_SOURCE_COPY,
    topLevelAllowlist: APP_COPY_TOP_LEVEL_ALLOWLIST,
  });
